...
- `SceneController` manages the UI scene canvas and handles its own resize logic independently
...
- Full-screen UI scenes render on the `#ui-stage` canvas via `SceneController`:
  ...
  - **ConfirmScene**: Modal-style confirmation dialogs
  - **InGameMenuScene**: Pause menu with game controls
- All scenes include a consistent `NavigationBar` component:
  - 104px height matching in-game HUD styling
  - Back button, scene title, profile/settings buttons, currency pills
  - Context-aware navigation with proper state transitions
- Scene backgrounds use `SceneBackground` component:
  - 5 themed color variations (blue, red, yellow, purple, green)
  - 60px grid overlay with subtle radial center glow
  - Consistent aesthetic across all scenes
- State management via `UIStateMachine`:
  - States: `IN_GAME`, `LOBBY`, `SHOP`, `PROFILE`, `SETTINGS`, `PLAY_MODES`, `CONFIRM`, `IN_GAME_MENU`
  - Transition types: `CROSS_FADE`, `SLIDE_LEFT`, `SLIDE_RIGHT`
  - Scene lifecycle: `mount()` → `update(dt)` → `render(ctx)` → `unmount()`
- Input handling:
  - Mouse events captured on canvas with coordinate scaling
  - Navigation bar checked first before scene-specific hit detection
  - ESC key handled contextually (return to game, close menu, etc.)
