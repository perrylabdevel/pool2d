...
- Thumbnails are generated by an offscreen Three.js pass in `Renderer3D.generateBallIcons(sizePx)`:
  ...
  - Caches by pixel size; regenerates when the renderer scale changes enough to alter chip size.
- Chip size matches the on‑screen ball diameter:
  - `Game` listens to `renderer:resized`, computes `--ball-chip-size` from `CONFIG.BALL_RADIUS * 2 * scale`, and updates CSS.
  - Icon generation runs at that target size and updates the HUD when complete.
